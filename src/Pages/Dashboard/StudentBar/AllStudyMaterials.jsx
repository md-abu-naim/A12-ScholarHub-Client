...
const AllStudyMaterials = () => {
    ...
    return (
        <div>
            <SectionTitle heading='view all study materials' subHeading='This is All Materials Page' />
            <div className="grid grid-cols-1  lg:grid-cols-3 gap-4">
                {
                    sessions.map((session, i) => <div key={i} className='w-full rel max-h-[300px] max-w-md  px-4 py-3 bg-[#1B1616] rounded-md shadow-md hover:scale-[1.05] transition-all flex flex-col'>
                        ...
                        <div className="flex bottom-0 flex-grow items-end justify-end mt-4">
                            <Link to={`/dashboard/study-materials/${session._id}`} ><CommonBtn title=' View Materials' /></Link>
                        </div>
                    </div>)
                }
            </div>
